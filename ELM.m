function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy, EVAL_train, EVAL_test] = ELM(X,Ytrain, Ynew, Ytest, NumberofHiddenNeurons, ActivF, C)
tail=size(X);
TrainingData=tail(1);
NumberofInputNeurons=tail(2);
%%%%%%%%%%% Calculate weights & biases
start_time_train=cputime;
%%%%%%%%%%% Random generate input weights InputWeight (w_i) and biases BiasofHiddenNeurons (b_i) of hidden neurons
InputWeigh=randn(NumberofHiddenNeurons,NumberofInputNeurons)*2-1;
InputWeight=orth(InputWeigh');
BiasofHiddenNeurons=orth(randn(NumberofHiddenNeurons,1))*2-1;
...
Weights=orth(Weigh');
BiasofHidden=orth(randn(NumberofHiddenNeurons,1));
tempH_test = X*Weights;
...
EVAL_train = Evaluate(Ytrai, Y);
EVAL_test = Evaluate(Yes, TY);
